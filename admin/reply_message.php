...
if (!isset($_GET['id'])) {
    header('Location: contacts_list.php'); // chuyển về trang danh sách nếu không có id
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($replyContent)) {
        ...
        // Giả sử bạn có hàm gửi SMS hoặc gửi mail trong service
        // sendReplyToUser($message['phone'], $replyContent);

        // Hoặc lưu trả lời vào database
...
<body>
    <section class="section-reply">
        ...
        <?php if ($replySent): ?>
            ...
            <a href="contacts_list.php">Quay lại danh sách tin nhắn</a>
        <?php else: ?>
            <?php if ($error): ?>
